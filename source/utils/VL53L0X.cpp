// VL53L0X Time-of-Flight distance sensor library for mbed
// Based on Pololu's Arduino library
...
{
    char data[3] = {(char)reg, (char)(value >> 8), (char)(value & 0xFF)};
...
{
    char data[5] = {(char)reg, (char)(value >> 24), (char)(value >> 16), (char)(value >> 8), (char)(value & 0xFF)};
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
    ...
    return ((uint32_t)data[0] << 24) | ((uint32_t)data[1] << 16) | ((uint16_t)data[2] << 8) | data[3];
}
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
    char r = reg;
    i2c->write(address, &r, 1, true);
    i2c->read(address, (char*)dst, count);
}
...
{
    timeout_start_ms = timer.read_ms();
    while ((readReg(RESULT_INTERRUPT_STATUS) & 0x07) == 0)
    {
        if (io_timeout > 0 && ((uint16_t)(timer.read_ms() - timeout_start_ms) > io_timeout))
        {
            did_timeout = true;
            return 65535;
        }
